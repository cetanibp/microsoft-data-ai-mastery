...
Run the scripts against an empty SQL database named `NorthstarControl` in the appropriate Fabric workspace. The scripts do not create a database, Fabric item, workspace, connection, or credential.
...
The later implementation should use distinct roles:
...
Direct updates to approved definition rows or historical audit events are prohibited by operating policy. Role DDL will be added with the ownership and lifecycle guidance after the test harness confirms the required operations.

## Validation boundary

Relational constraints reject invalid keys, enumerations, ranges, and unsafe literal shapes. `ctrl.vw_MetadataValidationIssue` adds cross-row semantic checks. Automated tests in the next increment will additionally validate:
